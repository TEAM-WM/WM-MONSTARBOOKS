...
<body>
			...
			<form action="${pageContext.request.contextPath}/join/termsAccess" method="post" class="terms left">
				<div class="term-all-box">
					<div>
						...
						<p class="agree-title">모두 동의합니다.</p>
					</div>
				</div>
				<div class="term-item-box">
					<div>
						<input type="checkbox" name="policyAgree" id="policyAgree" value="이용약관"/>
						<label for="policyAgree"></label>
						<p class="agree-title">
							이용약관
							<strong>
								(필수)
					...
					<button type="button" onclick="openModal('policyAgreeModal')">상세보기</button>
				</div>
				<div class="term-item-box">
					<div>
						<input type="checkbox" name="privacyAgree" id="privacyAgree" value="개인정보"/>
						<label for="privacyAgree"></label>
						<p class="agree-title">
							개인정보 수집 및 이용 동의
							<strong>
								(필수)
					...
					<button type="button" onclick="openModal('privacyAgreeModal')">상세보기</button>
				</div>
				<div class="term-item-box">
					<div>
						<input type="checkbox" name="limitAgree" id="limitAgree" value="만14세"/>
						<label for="limitAgree"></label>
						<p class="agree-title">
							만 14세 이상입니다.
							<strong>
								(필수)
	...
	<!-- 모달 영역 -->
	<div id="policyAgreeModal" class="modal terms-modal">
...
제24조(재판권 및 준거법)
    ...
    ② “몰”과 이용자 간에 제기된 전자상거래 소송에는 한국법을 적용합니다.
                ...
                <button class="closeModalBtn" onclick="closeModal('policyAgreeModal')">닫기</button>
            </div>
        </section>
    </div>

    <div id="privacyAgreeModal" class="modal terms-modal">
        <section class="modal-content-wrap">
            <div class="modal-title left">
                <h3>
                    MonstarBooks 개인정보 수집 및 이용동의
                </h3>
            </div>
            <div class="modal-content">
                <div class="modal-content-text left">
                    <pre>
    몬스타북스(이하 ‘몬스타북스’라 한다)는 개인정보 보호법 제30조에 따라 정보 주체의 개인정보를 보호하고 이와 관련한 고충을 신속하고 원활하게 처리할 수 있도록 하기 위하여 다음과 같이 개인정보 처리지침을 수립, 공개합니다.
    ...
    <strong>제2조(개인정보 처리방침 시행 및 변경)</strong>
    이 개인정보 처리방침은 2023. 08.01 부터 적용됩니다.
                    </pre>
                </div>
                <button class="closeModalBtn" onclick="closeModal('privacyAgreeModal')">닫기</button>
            </div>
        </section>
    </div>

    <div id="submitChkModal" class="modal">
        <section class="modal-content-wrap">
            <div class="modal-content">
                ...
                <button class="closeModalBtn" onclick="closeModal('submitChkModal')">닫기</button>
            </div>
        </section>
    </div>
    ...
    const otherCheckboxes = document.querySelectorAll("input[type='checkbox']:not(#termsAll)");
    const submitButton = document.querySelector('input[type="submit"]')

    // 전체 선택/해제 체크박스 상태에 따라 다른 체크박스 상태 업데이트
    function updateOtherCheckboxes() {
        let allChecked = true;
    ...
    // 다른 체크박스가 변경되었을 때 전체 선택 체크박스의 상태 업데이트
    otherCheckboxes.forEach(function(checkbox) {
    ...
    // Submit 버튼 클릭 시 체크 상태 확인 후 alert 창 띄우기
    submitButton.addEventListener("click", function(event) {
        if (!termsAllCheckbox.checked) {
            event.preventDefault(); // 폼 제출 방지
            openModal("submitChkModal"); // 모달 띄우기
        }
    });
});
