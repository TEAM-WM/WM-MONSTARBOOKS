...
<body>
	<h2>공지사항 관리</h2>
	<button onclick="insertNotice();">공지등록</button>
	<br /> 총 ${total } 건
	...
	<form action="notice" method="post">
		...
		<c:if test="${searchVo.page>1 }">
			<button>
				<a
					href="notice?page=1&sk=${sk }&ntitle=${ntitle==true?'ntitle':''}
		&ncontent=${ncontent==true?'ncontent':''}">처음으로</a>
			</button>
			<button>
				<a
					href="notice?page=${searchVo.page-1 }&sk=${sk}&ntitle=${ntitle==true?'ntitle':''}
		&ncontent=${ncontent==true?'ncontent':''}">이전으로</a>
			</button>
		</c:if>
		...
		<c:forEach begin="${searchVo.pageStart }" end="${searchVo.pageEnd }"
			...
			<c:choose>
				...
				<c:otherwise>
					<a
						href="notice?page=${i }&sk=${sk}&ntitle=${ntitle==true?'ntitle':''}
		...
		<c:if test="${searchVo.page < searchVo.totPage }">
			<button>
				<a
					href="notice?page=${searchVo.page+1 }&sk=${sk}&ntitle=${ntitle==true?'ntitle':''}
		&ncontent=${ncontent==true?'ncontent':''}">다음으로</a>
			</button>
			<button>
				<a
					href="notice?page=${searchVo.totPage }&sk=${sk}&ntitle=${ntitle==true?'ntitle':''}
		&ncontent=${ncontent==true?'ncontent':''}">끝으로</a>
			</button>
		</c:if>
		...
		<div>
			...
			내용 <input type="text" name="sk" value="${sk }" style="width: 150px;"
				maxlength="100" /> <input type="submit" value="검색" />
